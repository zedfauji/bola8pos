...
- ✅ **v2.2 — UI Standardization** — Phases 29-35 (+33.1) (shipped 2026-07-17) — see `.planning/milestones/v2.2-ROADMAP.md`
...
- [x] 07-09-PLAN.md — Atomic `start_pool_session` + `seat_waitlist_party_and_start_session` RPCs + `pg_net` extension, pushed live (Wave 1) (D-01, D-02, D-04, D-06, D-08)
...
- [x] 10-01-PLAN.md — Audit pipeline: install knip/jscpd/madge devDeps + `knip.json`/`.jscpd.json` + `scripts/run-tech-debt-audit.sh` + `audit:tech-debt` npm script, all 8 checks wired (Wave 1)

**Wave 2** *(blocked on Wave 1 completion)*

- [x] 10-02-PLAN.md — Structural-smell probes (`as any` / TODO-FIXME / file sizes) + full whole-codebase audit run + per-report digests (Wave 2)
...
- [x] 21-04-PLAN.md — Staff admin per-staff locale field: EditLocaleDialog + StaffDashboard column/action (Wave 3) (SC-2, SC-4, D-03)
- [x] 21-05-PLAN.md — Receipt + PDF locale (D-06): receipt-format/printer.rs/pos-printer/pdf.tsx, move label selection into TS (Wave 3) (SC-2, SC-4, D-06)
...
- [x] 21-10-PLAN.md — Sweep widgets (reports/admin + settings-tab contents) → wAdmin ns (Wave 4) (SC-4, D-04)
- [x] 21-11-PLAN.md — Sweep entities + pages → entities/pages ns (Wave 4) (SC-4, D-04)
...
- [x] 22-04-PLAN.md — `/edit-history` view (widget + page + route + Home tile) (SC-4)

**Wave 2** *(blocked on Wave 1 completion)*

- [x] 22-03-PLAN.md — `edit-paid-tab` feature slice (hook + dialog) + PaymentPane trigger (SC-3)
...
- [x] 23-01-PLAN.md — Foundations: register `tab.reopen` audit action + `reopen_tab` RBAC action, extend PaymentSchema/TabSchema, map payment status, Wave-0 test scaffolds (Wave 1)
...
- [x] 24-04-PLAN.md — Deletions RPCs + audited atomic `remove_tab_item` RPC (D-05/D-06/D-07, restore-before-delete, no role gate, audit_logs plural) (SC-1, SC-4)

**Wave 3**

- [x] 24-05-PLAN.md — [BLOCKING] `supabase db push` (5 migrations) + regenerate supabase.types.ts (SC-1, SC-4)

**Wave 4**

- [x] 24-06-PLAN.md — Report query hooks: migrate hourly/voids bodies to RPC + 4 new use*Report hooks + 4 live integration tests (SC-1, SC-4)
- [x] 24-07-PLAN.md — remove-tab-item feature: hook → RPC + required reason dialog, no PIN gate (SC-1, D-06/D-07)
...
- [x] 24-10-PLAN.md — ReportsPage 4-tab wiring + reports E2E (new tabs + CSV export + bartender reason removal) + CLAUDE.md + phase gate (SC-1..SC-4)
...
- [x] 25-01-PLAN.md — tracer: `groupByCategory`/`formatModifierLines` utility + `ReceiptDataSchema` fields + both receipt text builders (Wave 1) (SC-1, SC-2, SC-3)
...
- [x] 26-04-PLAN.md — waitlist "seat at new temporary table" action + floating badge + i18n + E2E (Wave 4) (SC-1, SC-3)
...
- [x] 27-02-PLAN.md — TRACER: `open_units` table + `products` linkage columns + `consume_open_unit` + `deplete_for_order_item` v5 + 6 audit actions, pushed and proven end-to-end (SC-1, SC-2, SC-4)
...
- [x] 27-05-PLAN.md — `OpenUnit*` Zod schemas + `ProductSchema` config fields + `@entities/open-unit` read/mutation hooks (SC-1, SC-3)
...
- [x] 27-07-PLAN.md — three features: open (ungated, D-11), correct + void (manager PIN gated, D-12) (SC-3)
...
- [x] 28-01-PLAN.md — format.ts core (formatMoney/formatMoneyIn/formatPercent/parseMoneyInput) + tests + MoneyDisplay wiring (tracer, Wave 1) (SC-1)
...
- [x] 36-04-PLAN.md — Human gate: native window opens, setup script is idempotent, pre-existing blockers get an owner (D-03, D-09, D-12)
