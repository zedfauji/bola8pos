...
/**
 * PromotionAvailabilityEditor
 *
 * Manages day-of-week + time window availability for a promotion.
 * Near-verbatim clone of ComboAvailabilityEditor.tsx (see 20-UI-SPEC.md §3) —
 * swaps comboId->promotionId, combo_availability->promotion_availability,
 * combo_product_id->promotion_id, comboKeys->promotionKeys.
 * Uses `const db = supabase as any` pre-regen cast — promotion tables not yet
...
function availabilityToWindowDraft(w: PromotionAvailability): WindowDraft {
  return {
    id: w.id,
  ...
  const saveMutation = useMutation({
    mutationFn: async (windowDrafts: WindowDraft[]) => {
      ...
      if (windowDrafts.length > 0) {
        ...
        const { error: insErr } = await db.from('promotion_availability').insert(rows);
