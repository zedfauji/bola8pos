 ...
 * openable before/after diff (mirrors e2e/38-audit-logs.spec.ts's DOM/a11y
...
// ---------------------------------------------------------------------------
// Seed helper — mirrors e2e/35-refund.spec.ts's seedPaidTab (single item).
// ---------------------------------------------------------------------------
...
async function editPaidTabViaUi(
  ...
  await row.getByRole('button', { name: 'Editar ticket' }).click();

  const dialog = page.getByRole('dialog', { name: 'Editar ticket pagado' });
  ...
  await expect(page.getByText(/corrección del ticket guardada/i)).toBeVisible({
    timeout: 15_000,
  });
...
test.describe('Edit Paid Tab', () => {
  ...
  test.describe('SC-4: /edit-history lists edits and opens the diff viewer', () => {
    test('manager sees the edit in /edit-history and row click opens the JsonDiffViewer', async ({
      ...
      await expect(page.getByRole('heading', { name: 'Historial de ediciones' })).toBeVisible({
        timeout: 15_000,
      });
