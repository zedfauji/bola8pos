 ...
 * setup-dev-users.ts
 *
 * Ensures the four PIN-login E2E staff accounts (admin/manager/bartender/kitchen)
...
// Matches e2e/helpers/auth.ts's StaffRole union.
const ROLES: { envPrefix: string; role: 'admin' | 'manager' | 'bartender' | 'kitchen' }[] = [
  { envPrefix: 'E2E_ADMIN', role: 'admin' },
  { envPrefix: 'E2E_MANAGER', role: 'manager' },
  { envPrefix: 'E2E_BARTENDER', role: 'bartender' },
...
async function ensureStaffAccount(
  role: 'admin' | 'manager' | 'bartender' | 'kitchen',
  name: string,
  pin: string
): Promise<void> {
