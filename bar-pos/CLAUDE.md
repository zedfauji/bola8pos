...
- Operational reports suite + CSV — 6 report RPCs: `get_peak_hours_report`/`get_voids_report` (migrated from client-side aggregation to bounded server-side RPCs), `get_deletions_pre_report`/`get_deletions_post_report`, `get_modifier_popularity_report` (unnest-CTE, avoids double-counting `order_items.modifier_ids` array elements), `get_payment_methods_report` (two-grain: per-caja-session + day rollup, inherits Phase 23's `reopened_void`/`is_refund`/`is_deleted` exclusions); "charts-data" has no separate endpoint — the peak-hours/modifier-popularity/payment-methods RPCs feed their own Recharts widgets directly. `remove_tab_item` atomic RPC (`order_item.remove` audit action, required reason) replaces `useRemoveTabItem`'s old 3-step client delete/check-remaining/void mutation — no new role or PIN gate was added to the RPC or `RemoveTabItemDialog` itself (its one caller, `TableStatusPanel`, retains its own pre-existing, unrelated manager-PIN wrapper around all pool-table item removal). Generic `rowsToCsv` CSV serializer (`shared/lib/exporters/csv.ts`) wired to all 17 report tabs, including 5 CSV-only exports with no Excel/PDF option yet (tip-split, deletions-pre, deletions-post, modifier-popularity, payment-methods). 4 new `/reports` tabs: `DeletionsPreSendPanel` (standing historical-gap `Alert`), `DeletionsPostCloseReport`, `ModifierPopularityReport` (Recharts horizontal bar, top-20 display cap), `PaymentMethodsReport` (Recharts donut + two-grain table) (SC-1..SC-4, Phase 24). Post-implementation UAT hardened 3 issues: `rowsToCsv`'s `sanitizeCsvCell()` neutralizes CSV formula injection (CWE-1236) by prefixing any cell value starting with `=,+,-,@,tab,CR` with a leading `'`; `MoneyDisplay` no longer double-prefixes `$` (it was rendering a literal `$` in front of `formatMoney()`'s already-`$`-prefixed output — a latent bug across all 44 call sites, not just Phase 24); `HourlyBreakdownPanel`'s `<Bar>` now sets `fill` explicitly so its recharts `<Legend>` swatch matches the chart instead of defaulting to black.
